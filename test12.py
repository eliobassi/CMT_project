...
def extract_mean_per_region(raster_path, regions):
    ...
    with rasterio.open(raster_path) as src:
        # Reproject the shapefile to match the raster's coordinate reference system (CRS)
...
# Drop rows where either NDVI or NO2 values are missing
...
# ------------------------------------------------------
# Initialize an empty list to store the fitting results
rows = []
...
# Load the fitted parameters to get the last known NO2 per region
df = pd.read_csv("fitted_parameters.csv")

# Get a list of all regions
regions = df["Region"].unique()

# Determine the last measured year in the dataset (this will be used for future predictions)
last_year = df["Year"].max()

# Define a range of future years for prediction (from the year after the last known year to 2050)
...
# ------------------------------------------------------
# Load the fitted parameters from the CSV file
...
clean_minus = merge_and_clean(scenario_minus, df_fitted, "NO2 -1%/year")
clean_plus  = merge_and_clean(scenario_plus, df_fitted, "NO2 +1%/year")
...
# Print the names of the final output files
